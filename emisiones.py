...
def cargar_emisiones():
    ...
    columnas_deseadas = ['ESTACION', 'MAGNITUD', 'ANO', 'MES'] + [f'D{i:02d}' for i in range(1, 32)]
    df = df[columnas_deseadas]

    # ii. Debe reestructurar el DataFrame para que los valores de los contaminantes de las columnas de los días aparezcan en una única columna. NOTA: Investigar función melt
    ...
    # iv. Eliminar los renglones con fechas no válidas (puede utilizar la función isnat del módulo numpy) y ordenar el DataFrame por estaciones contaminantes y fecha. (5%)
...
serie_emisiones = obtener_emisiones(df_emisiones, estacion_ejemplo, magnitud_ejemplo, fecha_inicio_ejemplo,fecha_fin_ejemplo)
